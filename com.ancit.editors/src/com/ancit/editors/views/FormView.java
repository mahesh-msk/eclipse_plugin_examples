 ...
 */
package com.ancit.editors.views;
...
public class FormView extends ViewPart {
	...
	public void createPartControl(Composite parent) {
		...
		rtext.setImage("image", Activator.getDefault().getImageRegistry().get(Activator.IMG_SAMPLE)); //$NON-NLS-1$
		 ...
		 * "Some text to put in the first column", SWT.WRAP); label =
		 ...
		 * , SWT.WRAP); td = new TableWrapData(); td.colspan = 2;
		 * label.setLayoutData(td); label = toolkit.createLabel(form.getBody(),
		 * "This text will span two rows and should not grow the column.", SWT.WRAP); td
		 * = new TableWrapData(); td.rowspan = 2; label.setLayoutData(td); label =
		 * toolkit.createLabel(form.getBody(),
		 * "This text goes into column 2 and consumes only one cell", SWT.WRAP);
		 * label.setLayoutData(new TableWrapData(TableWrapData.FILL_GRAB)); label =
		 * toolkit.createLabel(form.getBody(),
		 * "This text goes into column 3 and consumes only one cell too", SWT.WRAP);
		 * label.setLayoutData(new TableWrapData(TableWrapData.FILL)); label =
		 * toolkit.createLabel(form.getBody(),
		 * "This text goes into column 2 and consumes only one cell", SWT.WRAP);
		 * label.setLayoutData(new TableWrapData(TableWrapData.FILL_GRAB)); label =
		 * toolkit.createLabel(form.getBody(),
		 ...
		 * form.getBody().setBackground(form.getBody().getDisplay().getSystemColor(SWT.
